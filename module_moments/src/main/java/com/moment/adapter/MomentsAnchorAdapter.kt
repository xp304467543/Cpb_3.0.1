...
class MomentsAnchorAdapter(val mContext: Context) :
    ...
    inner class NineGridHolder : RecyclerView.ViewHolder {
        ...
        fun bind(gridInfo: NineGridInfoAnchor) {
            ...
            tvMomentsDiscussHolderTime?.text =
            ...
            imgMomentsDiscussHolderPhoto?.setOnClickListener {
                if (!FastClickUtil.isFastClick()){
                    if (gridInfo.getIsLive() == "1"){
                        Router.withApi(ApiRouter::class.java).toLive(gridInfo.getUserId().toString(),"",gridInfo.getUserName().toString(),
                            gridInfo.getIsLive().toString() ,"0","0","",gridInfo.getUserAvatar().toString())
                    }else   Router.withApi(ApiRouter::class.java).toAnchorPage(gridInfo.getUserId().toString())
