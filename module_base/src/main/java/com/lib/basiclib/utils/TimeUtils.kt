...
object TimeUtils {
    ...
    * long 类型转换成日期
    */
    fun longToDateStringYYMMDD(long: Long): String? {
        val date = Date(long * 1000)
        return formatYearMonthDayHourMinuteSecond.format(date)
    }
    ...
    fun secondToTime(long: Long):String{
        return when {
            long < 60 -> {
                String.format("00:%02d", long % 60)
            }
            long < 3600 -> {
                String.format("%02d:%02d", long / 60, long % 60)
            }
            else -> {
                String.format("%02d:%02d:%02d", long / 3600, long % 3600 / 60, long % 60)
            }
        }
    }
