...
object NetErrorEngine {
    ...
    private const val SERVER_ERROR_MSG = "连接超时,请稍后再试"
    private const val DATA_ERROR_MSG = "服务器异常"
    const val UNKNOW_MSG = "连接超时,请稍后再试"
    ...
    fun handleException(e: Throwable): ApiException {
        ...
        if (e is HttpException) {
            ...
            when (e.code()) {
                UNAUTHORIZED, FORBIDDEN, NOT_FOUND, REQUEST_TIMEOUT, GATEWAY_TIMEOUT, INTERNAL_SERVER_ERROR, BAD_GATEWAY, SERVICE_UNAVAILABLE -> exception.setMsg(
                    SERVER_ERROR_MSG
                )
