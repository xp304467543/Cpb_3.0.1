...
import com.customer.component.trendview.TrendViewHeadType
import com.customer.component.trendview.TrendViewType
...
object LotteryComposeUtil {
        ...
        if (total != null) {
            when (type) {
                LotteryConstant.TYPE_LUZHU_2, LotteryConstant.TYPE_LUZHU_15 -> {
                    val da =
                        if (total[position].asJsonObject.get("da") == null) "0" else total[position].asJsonObject.get(
                            "da"
                        ).asString
                    val xiao =
                        if (total[position].asJsonObject.get("xiao") == null) "0" else total[position].asJsonObject.get(
                            "xiao"
                        ).asString
                ...
                LotteryConstant.TYPE_LUZHU_3, LotteryConstant.TYPE_LUZHU_16 -> {
                    val dan =
                        if (total[position].asJsonObject.get("dan") == null) "0" else total[position].asJsonObject.get(
                            "dan"
                        ).asString
                    val shuang =
                        if (total[position].asJsonObject.get("shuang") == null) "0" else total[position].asJsonObject.get(
                            "shuang"
                        ).asString
                ...
                LotteryConstant.TYPE_LUZHU_8 -> {
                    val long =
                        if (total[position].asJsonObject.get("long") == null) "0" else total[position].asJsonObject.get(
                            "long"
                        ).asString
                    val hu =
                        if (total[position].asJsonObject.get("hu") == null) "0" else total[position].asJsonObject.get(
                            "hu"
                        ).asString
                    when (lotteryID) {
                        ...
                        "10", "1" -> {
                            val he =
                                if (total[position].asJsonObject.get("he") == null) "0" else total[position].asJsonObject.get(
                                    "he"
                                ).asString
                ...
                LotteryConstant.TYPE_LUZHU_9 -> {
                    val qian =
                        if (total[position].asJsonObject.get("qian") == null) "0" else total[position].asJsonObject.get(
                            "qian"
                        ).asString
                    val hou =
                        if (total[position].asJsonObject.get("hou") == null) "0" else total[position].asJsonObject.get(
                            "hou"
                        ).asString
                ...
                LotteryConstant.TYPE_LUZHU_11 -> {
                    val zonglai =
                        if (total[position].asJsonObject.get("_1") == null) "0" else total[position].asJsonObject.get(
                            "_1"
                        ).asString
                    val meilai =
                        if (total[position].asJsonObject.get("_0") == null) "0" else total[position].asJsonObject.get(
                            "_0"
                        ).asString
                ...
                LotteryConstant.TYPE_LUZHU_12 -> {
                    val hong =
                        if (total[position].asJsonObject.get("hong") == null) "0" else total[position].asJsonObject.get(
                            "hong"
                        ).asString
                    val lv =
                        if (total[position].asJsonObject.get("lv") == null) "0" else total[position].asJsonObject.get(
                            "lv"
                        ).asString
                    val lan =
                        if (total[position].asJsonObject.get("lan") == null) "0" else total[position].asJsonObject.get(
                            "lan"
                        ).asString
    ...
    fun getNumTrendMap(
        type: String,
        data: List<LotteryCodeTrendResponse>?,
        dataFrom: List<LotteryCodeTrendResponse>?,
        trendViewHead: TrendViewHeadType?,
        trendViewContent: TrendViewType?,
        trendContainer: LinearLayout?,
                ...
                LotteryConstant.TYPE_20, "前三形态", "中三形态", "后三形态" -> {
                    if (dataFrom != null) {
                        for ((index, it) in data.withIndex()) {
                            ...
                            map["date"] = it.issue
                            map["num0"] = it.open_code?.split(",")?.get(0)
                            map["num1"] = it.open_code?.split(",")?.get(1)
                            map["num2"] = it.open_code?.split(",")?.get(2)
