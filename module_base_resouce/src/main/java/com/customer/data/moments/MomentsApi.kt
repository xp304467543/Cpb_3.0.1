...
object MomentsApi : BaseApi {
    ...
    fun getHotDiscuss(
        limit: String,
        page: Int,
        function: ApiSubscriber<List<MomentsHotDiscussResponse>>.() -> Unit
    ) {
    ...
    fun getAnchorDynamic(
        ...
        val subscriber = object : ApiSubscriber<List<MomentsAnchorListResponse>>() {}
        subscriber.function()
        getApi().get<List<MomentsAnchorListResponse>>(HOME_LIVE_ANCHOR_ANCHOR_DYNAMIC)
     ...
     * 评论点赞
     */
    fun getQuizCommentLike(comment_id: String, function: EmptySubscriber.() -> Unit) {
        val subscriber = EmptySubscriber()
        subscriber.function()
        getApiLottery()
            .post<String>(QUIZ_COMMENT_LIKE)
            .cacheMode(CacheMode.NONE)
            .headers("Authorization", UserInfoSp.getTokenWithBearer())
     ...
     * 评论列表
     */
    fun getCommentOnList(
        articleId: String,
        page: Int,
        function: ApiSubscriber<List<CommentOnResponse>>.() -> Unit
    ) {
    ...
    fun getCommentList(
        dym_id: String,
        page: Int,
        function: ApiSubscriber<List<DavisCommentOnResponse>>.() -> Unit
    ) {
    ...
    fun clickZansDavis(
        typeClick: String,
        like_id: String,
        function: ApiSubscriber<Zan>.() -> Unit
    ) {
        val subscriber = object : ApiSubscriber<Zan>() {}
        subscriber.function()
        getApi()
            .post<Zan>(COMMENT_ZAN)
            .cacheMode(CacheMode.NONE)
            .headers("token", UserInfoSp.getToken())
            .params("user_id", UserInfoSp.getUserId())
