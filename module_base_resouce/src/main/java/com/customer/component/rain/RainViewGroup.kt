...
class RainViewGroup : ConstraintLayout {
    ...
    //表情图片
    private var mImgResId = R.mipmap.img_red_packet_3
        ...
        if (mIsRun) {
            ...
            run {
                ...
                while (i < mBitmapList.size) {
                    ...
                    mBitmapList[i].bitmap?.let { canvas.drawBitmap(it, mMatrix!!, mPaint) }
    ...
    private fun createItem(item: ItemEmoje? = null): ItemEmoje {
        ...
        if (itemEmoje == null) {
            itemEmoje = ItemEmoje()
            val array = arrayListOf(R.mipmap.img_red_packet_3,R.mipmap.img_red_packet_2,R.mipmap.img_red_packet_1)
            itemEmoje.bitmap = BitmapFactory.decodeResource(resources, array[Random().nextInt(2)])
        }
    ...
    var mIsIntercept = false //是否拦截子项点击事件 默认不拦截
    ...
    fun setIsIntercept(mIsIntercept: Boolean) {
