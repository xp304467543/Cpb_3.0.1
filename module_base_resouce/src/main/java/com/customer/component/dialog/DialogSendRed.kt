...
import kotlinx.android.synthetic.main.dialog_send_red_hor.*
 ...
 *
 */
class DialogSendRed(context: Context, val isHor: Boolean) : Dialog(context) {
        ...
        etRedEnvelopeRedNumber.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(p0: Editable?) {
                if (p0.toString() != "" && p0.toString().toInt() > 200) {
                    ToastUtils.showToast("红包最多个数为200")
                    etRedEnvelopeRedNumber.setText("200")
                }
                if (p0.toString() != "" && p0.toString().toInt() < 1) {
                    ToastUtils.showToast("红包最少个数为1")
