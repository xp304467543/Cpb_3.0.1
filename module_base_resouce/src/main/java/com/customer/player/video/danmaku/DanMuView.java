package com.customer.player.video.danmaku;
 ...
 * @ Describe
 */
public  class DanMuView extends DanmakuView implements IControlComponent {
...
//                .setCacheStuffer(new BackgroundCacheStuffer(), null)  // 绘制背景使用BackgroundCacheStuffer
    ...
    public void addDanmaku(String text, boolean isSelf) {
        ...
        danmaku.text = EmotionSpanBuilder.buildEmotionSpannable(getContext(),text);
        danmaku.priority = 1;  // 小于0可能会被各种过滤器过滤并隐藏显示
        ...
        danmaku.borderColor = isSelf ? Color.BLUE : Color.TRANSPARENT;
        addDanmaku(danmaku);
    }
     ...
     * 发送自定义弹幕
     */
    public void addDanmakuWithDrawable(SpannableStringBuilder stringBuilder, boolean isSelf) {
...
//        danmaku.padding = 5;
    ...
    private SpannableStringBuilder createSpannable(Drawable drawable,String str) {
        String text = "bitmap";
        Spannable spannable = EmotionSpanBuilder.buildEmotionSpannable(getContext(),str);
        ...
        spannableStringBuilder.append(" ").append(spannable);
