...
class LotteryLinePop(context: Context, listLine: List<LineCheck>,val pos: Int) :
    ...
    inner class LineAdapter : BaseRecyclerAdapter<LineCheck>() {
        override fun getItemLayoutId(viewType: Int) = R.layout.holder_line
        override fun bindData(holder: RecyclerViewHolder, position: Int, data: LineCheck?) {
            ...
            mLDNetPingService = NetPingManager(context, real, object : NetPingManager.IOnNetPingListener {
                    @SuppressLint("SetTextI18n")
                    override fun ontDelay(log: Long) {
                        tvMs.post {
                            tvMs.text = (log / 2).toString() + "ms"
                            if ((log / 2) > 100) {
                                holder.textColorId(R.id.tvLineMs, R.color.colorYellow)
                            } else {
                                holder.textColorId(R.id.tvLineMs, R.color.colorGreen)
                            }
                        }
                    }
